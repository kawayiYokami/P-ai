<template>
  <div class="space-y-3">
    ...
    <div class="text-xs opacity-60">仅保留最近 10 轮发送-接收日志，进程退出后自动清空。</div>
      ...
      >
        <div class="card-body p-3 space-y-2">
          ...
          <details class="collapse collapse-arrow bg-base-200 border border-base-300">
            <summary class="collapse-title text-xs py-2 min-h-0">Headers</summary>
          ...
          <details class="collapse collapse-arrow bg-base-200 border border-base-300">
            <summary class="collapse-title text-xs py-2 min-h-0">Response</summary>
            <div class="collapse-content text-xs">
              <pre class="whitespace-pre-wrap break-all">{{ toPretty(entry.response ?? null) }}</pre>
            </div>
          </details>

          <div v-if="entry.error" class="text-xs text-error break-all">
            {{ entry.error }}
          </div>
        </div>
      </div>
