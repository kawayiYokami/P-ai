...
对于 apply_patch、write、delete、update、move 这类文件编辑工具，必须说明实际文件变化：write 说明新增或整写了哪个文件及核心内容；delete 说明删除了哪个文件；move 说明原路径和目标路径；update 对比 old_preview 与 new_preview，指出具体替换导致的语法、逻辑、配置或文案变化。若修改是在修复语法错误，必须精确说明是哪种语法错误，例如缺逗号、括号不闭合、字符串未闭合、赋值/比较误用等。\n\
对于 apply_patch、write、delete、update、move，allow=false 只用于无法判断修改语义、删除/覆盖高价值内容、修改安全/权限/密钥/发布链路、或存在明确破坏性后果；普通小修、明确修复、文案调整应 allow=true，并在 review_opinion 说明实际效果。\n\
