---
name: pai-guide
description: 当用户不清楚自己正在使用的 pai 应用能做什么，或不理解应用中的关键概念与关系时，必须阅读我。适用于解释 pai 的能力边界、窗口分工、人格、组织、工具、Skill、MCP、委托、任务、记忆、多会话、后台会话、远程联系人、LLM、快捷键及它们之间的关系，并帮助用户判断下一步应该怎么使用这个应用。
---
...
- 它可以聊天、调用工具、组织人格、接入 Skill 与 MCP、维护任务和记忆、处理多个会话，还可以和远程联系人相关的会话一起工作。
...
- 设置页面是主要配置和管理入口。
- 一般在这里管理模型、工具、Skill、MCP、人格、组织、任务、记忆、外观和其他设置。
...
- `组织` 是人格之间的上下级关系，由人格自己承载。
- 每个人格记录自己的直接下级人格，主助理是整棵组织树的根。
- 组织用来表达分工：谁负责前台对话，谁负责执行、研究、整理或特殊流程。
- 组织不是独立的一层实体，没有单独的“部门”对象。

### 人格和组织是什么关系

- `人格` 是角色设定，`组织` 是这些角色之间的上下级关系。
- `人格` 决定一个智能体怎么说话、怎么做事、偏什么风格。
- `记忆` 让这个智能体保留长期有价值的信息。
- 可以把 `人格 + 记忆` 一起理解成一个可持续工作的“智能体”。
- 组织关系决定这些智能体之间谁能把任务委托给谁：只能委托给**直接下级人格**。
- 主助理是组织树的根，负责在对话窗口和用户直接对话，不可删除。
- `explorer` 等下级人格负责被委托的子任务。
- 用户可以自建人格并把它们挂进组织，承担不同工作。
...
- 用户可以把它理解成“当前人格把一部分工作分派给它的直接下级人格”。
...
- **多个委托可以同时进行**：可以同时发起多个委托到不同下级人格，它们各自独立运行，不需要串行等待。
...
- **人格权限**：每个人格可以配置权限模式（白名单或黑名单），分别控制可用的内置工具、Skill 和 MCP 工具。在设置页面的人格配置里管理。
...
- 设置页面是所有配置的统一入口：模型、工具、Skill、MCP、人格、组织、任务、记忆、通知、外观、热键等。
...
- `LLM` 是底层能力，不等于某个具体人格。
- `人格` 和 `组织` 是两件事：前者是角色设定，后者是角色之间的上下级关系。
- `人格 + 记忆` 更接近一个可以长期工作的智能体。
- 组织关系决定谁能把任务委托给谁：只能委托给直接下级人格。
- 预设人格（主助理、explorer 等）不可删除，负责核心对话和子任务分派。
- 用户可以自建人格并把它们挂进组织，承担不同工作。
- `工具`、`Skill`、`MCP` 是智能体工作时能使用的能力。
- `任务`、`会话`、`后台会话` 是这些工作被承载和推进的方式。
- `远程联系人` 是外部渠道接进来的对象，不等于当前直接使用应用的人。

如果用户问“最容易理解的关系是什么”，优先用这句：

- `LLM` 像底层大脑，`人格 + 记忆` 像一个智能体，`组织` 像岗位分工，`工具` 像手脚，`Skill` 像工作说明书，`MCP` 像外部扩展接口，`任务` 像长期待办，`会话` 像不同工作线程。
...
- 你是想先直接聊天，还是先配置模型？
- 你是想理解概念，还是想完成一个具体任务？
- 你现在卡在模型、工具、Skill、MCP、人格、组织，还是任务系统上？
...
- 先说结论：人格是角色设定，组织是人格之间的上下级关系。
- 再补一句：人格决定一个智能体怎么表现；组织决定它上面是谁、下面能委托谁。
- 如果需要再展开：委托只能发给直接下级人格，主助理是组织树的根。
