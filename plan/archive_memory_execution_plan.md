...
## 当前状态（已完成）
- [x] API 配置新增 `context_window_tokens` 滑条（默认 128K，最大 200K）
- [x] 会话新增 `last_user_at` 与 `last_context_usage_ratio`
- [x] 归档对象新增 `summary`
- [x] 用户发言前执行归档判定（30 分钟 + 30% / 82% 强制）
- [x] 强制归档期间通过状态事件提示“正在归档优化上下文...”
- [x] 归档失败自动回退最近三轮；仍超限则自动开启新对话
- [x] 归档摘要 + 记忆提炼（最多 7 条）并自动合并入记忆库
- [x] 每次发送前注入隐藏归档回顾问答
- [x] 摘要流程支持记忆召回注入
- [x] 增加归档阈值单测与发送期输入冻结
